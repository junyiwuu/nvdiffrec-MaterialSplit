...
class _MLP(torch.nn.Module):
    def __init__(self, cfg, loss_scale=1.0):
        ...
        # loss scale的意义是，例如如果用float16训练，梯度太小容易变成0.所以放大梯度可以让小gradient保留下来 

        net = (torch.nn.Linear(
            cfg['n_input_dims'], cfg['n_neurons'], bias=False)
                                                                , torch.nn.ReLU())
        ...
        if self.loss_scale != 1.0:
            ...
            # 这个是pytorch的钩子汉书，在整个模块完成反向传播的时候调用
            # grad_i: 模块输入的梯度。 grad_o: 模块输出的梯度
            # 意思就是把输入梯度乘上loss scale来放大
    ...
    def _init_weights(m):
        ...
        # weight初始化用的kaiming是RELU friednly:
        # 也就是说，对每一层的weight，他会根据输入维度初始化一个符合某个分布的小数
        # fan in 越大，weight值越小
